<?php

namespace App\Livewire\Dashboard\Account;

use App\Models\Withdrawal;
use Livewire\Attributes\Layout;
use Livewire\Component;
use Livewire\WithPagination;

#[Layout('layouts.app')]
class WithdrawalHistory extends Component {
    use WithPagination;

    public $search = '';
    public $status = '';
    public $perPage = 12;
    // public $transactions;

    public $showModal = false;
    public $selected = null;

    // protected $updatesQueryString = ['search', 'status', 'page'];

    public function updated($prop) {
        $this->resetPage();
    }

    public function mapCurrencyLabel($currency) {
        $nameMap = [
            'BTC'  => 'Bitcoin',
            'ETH'  => 'Ethereum',
            'LTC'  => 'Litecoin',
            'TRX'  => 'TRON',
            'USDTTRC20' => 'Tether',
            'USDERRC20' => 'Tether',
            'USDC' => 'USD Coin',
        ];
        
        return $nameMap[\strtoupper($currency)] ?? null;
    }

    public function showDetails($id){
        $this->selected = Withdrawal::where([
            'user_id' => auth()->id(),
            'id' => $id
        ])->first();
        $this->showModal = (bool) $this->selected;
    }

    public function closeModal() {
        $this->showModal = false;
        $this->selected = null;
    }

    public function getTransactionsQueryProperty() {
        $q = Withdrawal::query()
            ->where('user_id', auth()->id())
            ->with(['currency', 'chain'])
            ->when($this->search, function ($builder) {
                $builder->where(function ($b) {
                    $b->where('address', 'like', '%' . $this->search . '%')
                    ->orWhere('tx_hash', 'like', '%' . $this->search . '%')
                    ->orWhere('amount', 'like', '%' . $this->search . '%')

                    // 🔹 Search by currency code or name
                    ->orWhereHas('currency', function ($q) {
                        $q->where('code', 'like', '%' . $this->search . '%')
                            ->orWhere('name', 'like', '%' . $this->search . '%');
                    })

                    // 🔹 Search by network name
                    ->orWhereHas('chain', function ($q) {
                        $q->where('name', 'like', '%' . $this->search . '%')
                            ->orWhere('chain', 'like', '%' . $this->search . '%');
                    });
                });
            })
            ->when($this->status, fn ($builder) =>
                $builder->where('status', $this->status)
            )
            ->orderByDesc('created_at');

        return $q;

    }

    public function getTransactionsProperty() {
        return $this->transactionsQuery->paginate($this->perPage);
    }

    public function render() {
        return view('livewire.dashboard.account.withdrawal-history', [
            'transactions' => $this->transactions,
            'totalAmount' => $this->transactionsQuery->sum('amount'),
        ]);
    }

}