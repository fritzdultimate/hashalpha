...
#[Layout('layouts.app')]
class LeaderboardComingSoon extends Component {
    public $activeTab = 1;
    public $leaderboard = [];
    public $challenge;
    public $myRank = null;
    ...
    public function setTab($id, $show = false) {
        $this->showPushBonus = $show;
        $this->activeTab = $id;
    ...
    public function loadLeaderboard() {
        ...
        if($this->showPushBonus) {
            $this->leaderboard = ChallengeFortyEightEntry::with('user')
    ...
    private function detectMyRank($category) {
        $entry = null;
        if($this->showPushBonus) {
            $entry = ChallengeFortyEightEntry::where('challenge_category_id', $category->id)
    ...
    public function render() {
        return view('livewire.dashboard.affiliate.leaderboard-coming-soon');
    }
}
