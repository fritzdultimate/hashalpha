<div class="halpha-space-y-6">
    ...
    <div class="halpha-flex halpha-gap-2 halpha-flex-wrap">
        ...
        @foreach($categories as $ch)
            ...
            <button 
                ...
                class="halpha-px-3 halpha-py-1 halpha-rounded halpha-text-xs
                {{ $activeTab == $ch->id && !$showPushBonus ? 'halpha-bg-accent-2 halpha-text-black' : 'halpha-bg-gray-800 halpha-text-gray-400' }}"
            >
                {{ $ch->challenge->name }}
            </button>
        @endforeach
    ...
    @if($selectedCategory->type === 'fastest' && collect($leaderboard)->where('score', '>=', 7)->count() > 2)
        <div class="halpha-bg-green-500/10 halpha-border halpha-border-green-400/30 halpha-rounded-lg halpha-p-3 halpha-text-center">
        ...
        @forelse($leaderboard as $entry)
            ...
            >
                ...
                <div class="halpha-relative halpha-flex halpha-justify-between halpha-items-center halpha-p-3 halpha-rounded-xl halpha-border {{ $applyStyle ? $rankStyles : 'halpha-bg-card-soft halpha-border-transparent' }}">
                    ...
                    {{-- Score --}}
                    @if($selectedCategory->type === 'fastest' && false)
